...
func run() {
	...
	worldMap[state.GS] = world.NewGameWorld(pixel.R(0, 0, 1024, 768))
	...
	for !win.Closed() {
		...
		ttscreen.DrawText(win.Canvas(), []string{fmt.Sprintf("FPS:%.2f", 1/s)}, pixel.V(30, 20), colornames.Green, 0) // TODO: remove this, just for debugging
		win.Update()
